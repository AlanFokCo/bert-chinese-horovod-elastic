...
def main():
    ...
    if (len(sys.argv) != 4 and len(sys.argv) != 5) or sys.argv[1] not in [
        ...
        "convert_transform_xl_checkpoint",
        "convert_gpt2_checkpoint",
    ]:
        print(
            "Should be used as one of: \n"
            ">> `pytorch_pretrained_bert convert_tf_checkpoint_to_pytorch TF_CHECKPOINT TF_CONFIG "
            "PYTORCH_DUMP_OUTPUT`, \n"
            ">> `pytorch_pretrained_bert convert_openai_checkpoint OPENAI_GPT_CHECKPOINT_FOLDER_PATH "
            "PYTORCH_DUMP_OUTPUT [OPENAI_GPT_CONFIG]`, \n"
            ">> `pytorch_pretrained_bert convert_transfo_xl_checkpoint TF_CHECKPOINT_OR_DATASET PYTORCH_DUMP_OUTPUT ["
            "TF_CONFIG]` or \n"
        ...
        if sys.argv[1] == "convert_tf_checkpoint_to_pytorch":
            ...
            if len(sys.argv) != 5:
                ...
                print(
                    "Should be used as `pytorch_pretrained_bert convert_tf_checkpoint_to_pytorch TF_CHECKPOINT "
                    "TF_CONFIG PYTORCH_DUMP_OUTPUT`")
            else:
            ...
            convert_openai_checkpoint_to_pytorch(OPENAI_GPT_CHECKPOINT_FOLDER_PATH,
                                                 OPENAI_GPT_CONFIG,
                                                 PYTORCH_DUMP_OUTPUT)
        elif sys.argv[1] == "convert_transform_xl_checkpoint":
            try:
